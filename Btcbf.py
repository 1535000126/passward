...
# 检查是否存在 cache.txt 文件，用于保存中断续跑的进度
...
class Btcbf():
    ...
    def speed(self):
        while True:
            if self.cur_n != 0:
                ...
                rate = (n - self.prev_n) / 2 if elapsed_t > 0 else 0  # 更新速率计算
                print("current n: " + str(n) + ", current rate: " + str(rate) + "/s" +
    ...
    def random_brute(self, n):
        ...
        if key.address in self.loaded_addresses:
            ...
            with open("foundkey.txt", "a") as f:  # 将找到的私钥和地址保存到 "foundkey.txt"
                f.write(key.address + "\n")
                f.write(key.to_wif() + "\n")
            sleep(510)
            exit()
...
if __name__ == "__main__":
    ...
    try:
        ...
        t0.start()
        t1.start()
        sleep(4000000)  # 保持在 try..except 中
